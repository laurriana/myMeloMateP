...
with open("/Users/lauriana/portfolio/myMeloMateP/demos/data/json/response-all-tracks.json", "r") as f:
    all_tracks = json.load(f)

track_data = {}
for t in all_tracks:
    track_data[t['id']] = t['artist']
...
for track, artist in track_data.items():
...
# save the csv
with open("track_artist_data.csv", "w", newline='') as fi:
    headers = ['track_id', 'artist_id']
    writer = csv.DictWriter(fi, fieldnames=headers)
    writer.writeheader()
    for t in db:
        writer.writerow({'track_id': t, 'artist_id': db[t]})